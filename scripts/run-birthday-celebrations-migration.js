/**
 * お祝い済み管理用テーブル作成マイグレーション
 * 使い方: SUPABASE_DB_PASSWORD=あなたのDBパスワード node scripts/run-birthday-celebrations-migration.js
 */
...
async function run() {
  ...
  if (!password) {
    ...
    console.error(
      "使い方: SUPABASE_DB_PASSWORD=あなたのパスワード node scripts/run-birthday-celebrations-migration.js"
    );
    process.exit(1);
  }
  ...
  try {
    ...
    console.log("  お誕生日一覧で「お祝い済み」チェックが利用可能になりました。");
  } catch (err) {
